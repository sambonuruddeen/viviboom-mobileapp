...
export default function GroupTabScreen({ navigation }: ChatRootTabScreenProps<'GroupTabScreen'>) {
  ...
  const { loading, loadMore, messages, refreshing, refreshList, reset } = usePaginatedSearchedMessages(searchQuery, filters);

  const memoizedFilters = useMemo(() => ({ ...filters, $and: [{ members: { $in: [chatClient?.user?.id] } }] }), [chatClient?.user?.id]);
  ...
  return (
    <View style={styles.container}>
      <ChatScreenHeader title="Channels" />
      <View
        style={[
