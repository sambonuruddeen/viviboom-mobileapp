...
import { Avatar, CircleClose, Delete, User, UserMinus, useTheme, vh, vw } from 'stream-chat-expo';
...
export const ChannelInfoOverlay = (props: ChannelInfoOverlayProps) => {
  ...
  const maxWidth = channel ? Math.floor(width / 8 - Object.keys(channel.state.members || {}).length.toString().length) : 0;
  const channelName = channel
    ...
    : '';
  const otherMembers = channel ? Object.values(channel.state.members).filter((member) => member.user?.id !== clientId) : [];
                        ...
                        <Text style={[styles.channelStatus, { color: grey }]}>
                          {otherMembers.length === 1
                            ? otherMembers[0].user?.online
                              ? 'Online'
                              : `Last Seen ${DateTime.fromISO(otherMembers[0].user?.last_active).toRelative()}`
                            : `${Object.keys(channel.state.members).length} Members, ${
                              Object.values(channel.state.members).filter((member) => !!member.user?.online).length
                            } Online`}
                        </Text>
                        <FlatList
