...
import { Avatar, ChatContext, Spinner, useTheme, vw } from 'stream-chat-expo';
...
export const MessageSearchList: React.FC<MessageSearchListProps> = React.forwardRef(
  (props, scrollRef: React.Ref<FlatList<MessageResponse<StreamChatGenerics>> | null>) => {
    const { EmptySearchIndicator, loading, loadMore, messages, refreshing, refreshList, showResultCount = false } = props;
    const {
      theme: {
      ...
      <>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              <Avatar channelId={item.channel?.id} id={item.user?.id} image={item.user?.image} name={item.user?.name} online={item?.user?.online} size={40} />
