...
export const UserSearchResults: React.FC<UserSearchResultsProps> = ({
  ...
  const { loading, loadMore, results: resultsContext, searchText, selectedUserIds, toggleUser } = useUserSearchContext();
  ...
  const {
    theme: {
      colors: { accent_blue, bg_gradient_end, bg_gradient_start, black, border, grey, grey_gainsboro, white_smoke, white_snow },
    },
  } = useTheme();
        ...
        <SectionList
          ...
          ListEmptyComponent={() => (
            <View style={styles.emptyResultIndicator}>
              ...
              <Text style={[{ color: grey }, styles.emptyResultIndicatorText]}>{loading ? 'Loading...' : 'No user matches these keywords...'}</Text>
            </View>
          )}
            ...
            >
              ...
              <View style={styles.searchResultUserDetails}>
                ...
                {showOnlineStatus && (
                  ...
                  >
                    Last online {DateTime.fromISO(item.last_active).toRelative()}
                  </Text>
                )}
              </View>
              {selectedUserIds.indexOf(item.id) > -1 && <>{removeOnPressOnly ? <Close pathFill={black} /> : <CheckSend pathFill={accent_blue} />}</>}
            </TouchableOpacity>
          )}
