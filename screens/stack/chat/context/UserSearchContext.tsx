...
export const UserSearchProvider = (props: React.PropsWithChildren<{ value?: UserSearchContextValue }>) => {
  const { children, value } = props;
  const paginatedUsers = usePaginatedUsers();

  const userSearchContext = { ...paginatedUsers, ...value };
  return <UserSearchContext.Provider value={userSearchContext as UserSearchContextValue}>{children}</UserSearchContext.Provider>;
};
