...
import { Avatar, useChannelPreviewDisplayName, useOverlayContext, useTheme } from 'stream-chat-expo';
...
export default function GroupChannelDetailsScreen({
  navigation,
  route: {
    params: { channel },
  },
}: ChatRootStackScreenProps<'GroupChannelDetailsScreen'>) {
  const { chatClient } = useChatContext();
  const { setOverlay: setChatOverlay } = useChatOverlayContext();
  ...
  const [muted, setMuted] = useState(chatClient?.mutedChannels.some((mute) => mute.channel?.id === channel?.id));
  ...
  const channelCreatorId = channel.data && (channel.data.created_by_id || (channel.data.created_by as UserResponse)?.id);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: white }]}>
      <ScreenHeader
        inSafeArea
        RightContent={() => (channelCreatorId === chatClient?.user?.id ? (
