...
import { ChatRootStackScreenProps } from '../types';
...
export default function OneOnOneChannelDetailScreen({
  navigation,
  route: {
    params: { channel },
  },
}: ChatRootStackScreenProps<'OneOnOneChannelDetailScreen'>) {
  const {
    theme: {
  ...
  const member = Object.values(channel.state.members).find((channelMember) => channelMember.user?.id !== chatClient?.user?.id);

  const user = member?.user;
  const [muted, setMuted] = useState(chatClient?.mutedUsers && chatClient?.mutedUsers?.findIndex((mutedUser) => mutedUser.target.id === user?.id) > -1);
  const [notificationsEnabled, setNotificationsEnabled] = useState(
    chatClient?.mutedChannels && chatClient.mutedChannels.findIndex((mutedChannel) => mutedChannel.channel?.id === channel.id) > -1,
  );
  ...
  return (
    <SafeAreaView style={[{ backgroundColor: white }, styles.container]}>
      <ScrollView contentContainerStyle={styles.contentContainer} style={styles.container}>
        ...
        <TouchableOpacity
          onPress={() => {
            navigation.navigate('SharedGroupsScreen', { user });
          }}
          style={[
            styles.actionContainer,
            {
