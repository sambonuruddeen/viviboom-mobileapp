...
interface MemberProjectTabProps {
  member: User;
...
const MemberProjectTab = forwardRef<ScrollView, MemberProjectTabProps>(({ member, scrollY, filterKey, setFilterKey, onScrollEnd }, ref) => {
  ...
  const fetchProjects = useCallback(
    async (hardRefresh = false) => {
      ...
      const requestParams = {
        ...
        ...(filters.find((f) => f.key === filterKey).params || {}),
      };
    ...
    [isFetchingProjects, isEndOfProjects, authToken, projects, member?.id, filterKey],
  );
  ...
  const flatListRenderItem = ({ item, index }) => <ProjectListItem id={item.id} preloadedData={item} />;
  ...
  const flatListHeaderComponent = useMemo(
      ...
      <View style={styles.listHeader}>
        <View style={styles.filterHeader}>
          <View style={styles.orderContainer}>
            {filters.map((filter) => (
              <TouchableOpacity key={`filter_${filter.key}`} style={styles.filterButton} onPress={() => setFilterKey(filter.key)}>
                <MyText
                  style={{
                    ...styles.filterButtonText,
                    color: filterKey === filter.key ? Colors[colorScheme].tint : '#666',
                    fontWeight: filterKey === filter.key ? '500' : '400',
                  }}
                >
                  {filter.key}
                </MyText>
    ...
    [colorScheme, filterKey, setFilterKey],
  );
    ...
    <Animated.FlatList
      ...
      ListFooterComponent={
        !isEndOfProjects ? null : (
          <MyText style={styles.noItemFoundText}>{!isFetchingProjects && !projects.length ? 'Nothing found here' : 'Yay! You have seen it all!'}</MyText>
        )
      }
      data={projects}
      renderItem={flatListRenderItem}
      onEndReached={() => fetchProjects(false)}
      keyExtractor={(item) => `badge-project_${item.id}`}
...
export default MemberProjectTab;
